...
- (void)initialize {
    ...
    _currentPage = 0;
    _controlSize = 6*kXX;
    _controlSpacing = 6*kXX;
    _otherColor = UIColorFromRGB(0xF5F5F5);
    _currentColor = UIRGBColor(68, 146, 255);
}
...
- (void)setDirectionType:(GBYPageControlDirectionType)directionType {
...
- (void)createPointView {
    ...
    for (int page = 0; page < _numberOfPages; page++) {
        if(page == _currentPage) {
            ...
            currPointView.layer.cornerRadius = 2*kYY;
            ...
            otherPointView.layer.cornerRadius = 2*kYY;
